...
public class AbstractAvr implements Cloneable {
  ...
  public int hashCode() {
    return Objects.hash(this.data,
        this.bitcopyFlag, this.carryFlag, this.overflowFlag, this.signFlag,
        this.interruptFlag, this.negativeFlag, this.zeroFlag, this.halfCarryFlag,
        Integer.valueOf(this.programCounter));
  }
  ...
  /**
   * returns the opcode of the next instruction.
   *
   * @return opcode op
   */
  public AvrInstruction.Opcode opCode() {
    AvrInstruction.Opcode op = decode(this.programCounter).getOpcode();
    assert op != null;
    return op;
  }
  ...
  private @Nullable AbstractAvr execute(AvrInstruction.BRBC insn) {
    ...
    if (bit == UNKNOWN) {
      fork = this.clone();
      fork.programCounter = (this.programCounter + insn.k + 1);
  ...
  private @Nullable AbstractAvr execute(AvrInstruction.SBR insn) {
    this.programCounter = this.programCounter + 1;
    Byte rd = this.data.read(insn.Rd);
    Byte rd2 = rd.or(Byte.from((byte) insn.K));
    this.data.write(insn.Rd, rd2);
  ...
  private void checkAddressKnown(Word address) {
    if (address.isUnknown()) {
      ...
      throw new IllegalArgumentException("Unknown indirect address" //$NON-NLS-1$
          + " (PC=0x" + pcHex + RBRACE); //$NON-NLS-1$
    }
  }
